...
$pdf->Cell(0,7,'Report Transfer from Stock Number',0,0,'C');
//New line
$pdf->Ln();
...
$sql="SELECT I.StockNo,Date,L.ProductCode,L.QuantityIn,L.QuantityOut FROM inventory I JOIN inventoryline L ON I.StockNo=L.StockNo WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and (L.StockNo LIKE 'TR%')";
$result = mysql_query($sql,$conn);
...
$sql="SELECT sum(QuantityIn), sum(QuantityOut) ".
			"FROM InventoryLine L JOIN Inventory I ON L.StockNo = I.StockNo ".
			"WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and I.StockNo like '%TR%'";
    $sumResult = mysql_query($sql,$conn);
    list($QuantityIn, $QuantityOut) = mysql_fetch_row($sumResult);
...
//report Header
//$pdf->SetTextColor(255,0,0);
$pdf->Cell(0,7,'Transfer In and Transfer Out Report by Month and Year',0,0,'C');
//New line
$pdf->Ln();
...
$pdf->Cell($w[2],5,$row['ProductCode'],L,0,'L',$fill);
...
{
$sql="SELECT P.ProductCode,P.ProductName,ProductType,L.QuantityIn,L.QuantityOut FROM Product P JOIN inventoryline L ON P.ProductCode = L.ProductCode JOIN Inventory I ON I.StockNo = L.StockNo WHERE month(Date) ='$month3' and year(Date) ='$year3' and (L.StockNo LIKE 'TR%')";
$result = mysql_query($sql,$conn);
...
$sql="SELECT sum(QuantityIn), sum(QuantityOut) ".
			"FROM InventoryLine L JOIN Inventory I ON L.StockNo = I.StockNo ".
			"WHERE month(Date) = '$month3' and year(Date) ='$year3' and I.StockNo like '%TR%'";
    $sumResult = mysql_query($sql,$conn);
    list($QuantityIn, $QuantityOut) = mysql_fetch_row($sumResult);
...
//report Header
//$pdf->SetTextColor(255,0,0);
$pdf->Cell(0,7,'Transfer In and Transfer out Report By Product Code',0,0,'C');
//New line
$pdf->Ln();
