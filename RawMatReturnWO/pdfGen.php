...
require('fpdf.php');
	...
	if($choice == 1)
	{
		$sql ="SELECT L.StockNo, L.ProductCode,P.ProductName, L.QuantityIn,WorkOrder
...
$header=array('Product Code','Product Name','Quantity In','WorkOrder');
$pdf->AddPage();
...
$pdf->Cell($w[3],5,$row['WorkOrder'],LR,0,'C',$fill);
...
$pdf->Output();
	...
	else if($choice == 2)
	{
		...
		$sql="SELECT L.StockNo, Date, ProductCode, QuantityIn, WorkOrder
FROM Inventory I
JOIN InventoryLine L ON I.StockNo = L.StockNo WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and (L.StockNo LIKE '%RW0%')";
    $result = mysql_query($sql,$conn);

    while($data=mysql_fetch_assoc($result))
				$listData[]=$data;
				
		$sql="SELECT SUM(L.QuantityIn) FROM Inventory I JOIN InventoryLine L ON I.StockNo=L.StockNo WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and (L.StockNo LIKE  '%RW0%');" ;
    $sumResult = mysql_query($sql,$conn);
...
$pdf=new FPDF();
//Column titles  InvoiceDate,TotalAmount,VAT,AmountDue
$header=array('Stock No','Date','Product Code','Quantity In','Work Order');
$pdf->AddPage();
...
//        $pdf->Cell($w[$i],7,$header[$i],'TB',0,'C');
    ...
    foreach($listData as $row)
    {
        ...
        $pdf->Cell($w[4],6,$row['WorkOrder'],LR,0,'C',$fill);
