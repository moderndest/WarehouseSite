...
<title>Raw Material Return WO</title>
...
if($OPENING)
	...
	//SELECT NEW LINEITEM
	  $sql = "SELECT P.ProductCode,ProductName,ProductType,QuantityIn,WorkOrder FROM InventoryLine I JOIN Product P ON I.ProductCode = P.ProductCode WHERE StockNo ='".$StockNo."'";
	  $result=mysql_query($sql);
	  //GET DATA
	...
	{ 
		...
		else
		{
			...
			$_SESSION['Reason'] = "Raw Material Return WO";
		}
		
	}
if($_SESSION['StockNo'] == "NEW")
		{	
			$Reason ="Raw Material Return WO";
...
Raw Material Return WO</h1>
<form id="form1" name="form1" action="">
  
  <!-- HEAD BUTTON -->
  <input type="button" value="NEW" onClick="window.location = 'clearReturnWO.php'" style="width:65px;">
<input type="submit" value="SAVE" formaction = "saveReturnWO.php" formmethod = "get" formtarget = "_blank"/>
<input type="submit" name="btnOpen" id="btnOpen" value="OPEN" style="width:65px;" onclick="window.open('ListofStockReturnWO.php','popup','width=610,height=500,scrollbars');"/>
<input type="submit" value="DELETE" formaction="deleteReturnWO.php" formmethod="get" formtarget="_blank" style="width:65px;"/>
...
<input type="button" name="Back" id="Back" value="BACK" />
  ...
  <table width="900" border="0">
    ...
    <tr>
      ...
      <input type="button" name="btCustomer" id="btCustomer" value="..." onclick="window.open('ListofSupplierReturnWO.php','popup','width=610,height=500,scrollbars');">
      </td>
      <td>Supplier Name:</td>
    ...
    if(isset( $_REQUEST['cmd']) && $_REQUEST['cmd']=='Delete') 
{
	...
	//IF CLICK BUTTON ADD
  if(isset($_REQUEST['btAdd']) && $_POST['txtProductCode'] != "" && $_POST['txtWorkOrder'] != "" && $_POST['txtQuantityIn'] == "")
{
	$duplicate = false;
	//GET DATA
...
else if( $_REQUEST['btAdd'] && $_POST['txtProductCode'] != "" && $_POST['txtQuantityIn'] != "" && $_POST['txtWorkOrder'] != "" )
{
	$duplicate = false;
...
//	echo "<script type=\"text/javascript\">";
//	echo "alert(\"Please input WorkOrder or QuantityIn \");";
//	echo "window.history.back();";
//	echo "</script>";
// } */
...
<?php
  ...
  $NoRow=1;
    ...
    <td align="center"><input name="EditQuantityIn" type="text" style="text-align:right; width:80px;" id="EditQuantityIn" value="<?=$EditQuantityIn?>" size="10" onkeyup="checkError('EditQuantityIn');" <?php if($selected == "Adjust in" || $selected == "select") echo 'class = "bg" readonly="readonly"'; ?>/></td>
  ...
  <tr>
   <td>&nbsp;</td>
    ...
    <td><label for="txtProductCode"></label>
      ...
      <input type="button" name="btProduct" id="btProduct" value="..." onclick="window.open('ListofProductReturnWO.php','popup','width=610,height=500,scrollbars');" /></td>
    ...
    <td><label for="txtWorkOrder"></label>
      ...
      <input type="button" name="btWorkOrder" id="btWorkOrder" value="..." onclick="window.open('ListofWorkOrderReturnWO.php','popup','width=610,height=500,scrollbars');" /></td>
    </tr>
