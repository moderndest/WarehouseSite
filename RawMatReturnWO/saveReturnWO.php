...
if($txtStockNo == 'NEW')
{
	$sql = "SELECT * FROM `runningnumber` WHERE Name = 'RW'";
	...
	$number=$data[1];
	$number++;
	$sql="UPDATE runningnumber SET Max = {$number} WHERE Name= 'RW'";
	//echo $sql;
    $result=mysql_query($sql);	 
}
else
...
for ($i=0; $i<=10; $i++)
	{
		if($LineItems[$i]["ProductCode"] != '')
		{
			...
			$sql_line = "INSERT INTO `InventoryLine`(`StockNo`, `ProductCode`, `QuantityIn`, `WorkOrder`) VALUES ('{$txtStockNo}','{$ProductCode}','{$QuantityIn}','{$WorkOrder}')";
			//echo "<br>";
			//echo "<br>";
			//echo $sql_line;
			$NoRow++;
...
$print ="window.opener.location.href=\"mainReturnWO.php?StockNo=".$txtStockNo."\";";
