...
if($txtStockNo == 'NEW')
{
	$sql = "SELECT * FROM `runningnumberpo` WHERE Name = 'WK'";
	...
	$number=$data[1];
	$number++;
	$sql="UPDATE runningnumberpo SET Max = {$number} WHERE Name= 'WK'";
	//echo $sql;
    $result=mysql_query($sql);	 
}
else
...
for ($i=0; $i<=10; $i++)
	{
		if($LineItems[$i]["ProductCode"] != '')
		{
			...
			$sql_line = "INSERT INTO `InventoryLine`(`StockNo`, `ProductCode`,  `Price`,`QuantityIn`,`TotalPrice`, `WorkOrder`) VALUES ('{$txtStockNo}','{$ProductCode}','{$UnitPrice}','{$QuantityIn}','{$TotalPrice}','{$PONo}')";
			//echo "<br>";
			//echo "<br>";
			//echo $sql_line;
			$NoRow++;
...
$print ="window.opener.location.href=\"WO.php?StockNo=".$txtStockNo."\";";
