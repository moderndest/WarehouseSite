...
if($txtStockNo == 'NEW')
{
	$sql = "SELECT * FROM `runningnumber` WHERE Name = 'IM'";
	...
	$number=$data[1];
	$number++;
	$sql="UPDATE runningnumber SET Max = {$number} WHERE Name= 'IM'";
	//echo $sql;
    $result=mysql_query($sql);	 
}
else
...
for ($i=0; $i<=10; $i++)
	{
		if($LineItems[$i]["ProductCode"] != '')
		{
			...
			$sql_line = "INSERT INTO `InventoryLine`(`StockNo`, `ProductCode`, `QuantityOut`, `ManufactureOrder`) VALUES ('{$txtStockNo}','{$ProductCode}','{$QuantityOut}','{$ManufactureOrder}')";
			//echo "<br>";
			//echo "<br>";
			//echo $sql_line;
			$NoRow++;
