<?php
	...
	if($_POST['txtManufactureOrder']!=NULL)	
...
<title>Raw Material Issue MO</title>
...
if($OPENING)
	...
	//SELECT NEW LINEITEM
	  $sql = "SELECT P.ProductCode,ProductName,QuantityOut,ManufactureOrder FROM InventoryLine I JOIN Product P ON I.ProductCode = P.ProductCode WHERE StockNo ='".$StockNo."'";
	  $result=mysql_query($sql);
	  //GET DATA
	...
	{ 
		...
		else
		{
			...
			$_SESSION['Reason'] = "Raw Material Issue MO";
		}
		
	}
if($_SESSION['StockNo'] == "NEW")
		{	
			$Reason ="Raw Material Issue MO";
    ...
    if(isset( $_REQUEST['cmd']) && $_REQUEST['cmd']=='Delete') 
{
	...
	//IF CLICK BUTTON ADD
  if(isset($_REQUEST['btAdd']) && $_POST['txtProductCode'] != "" && $_POST['txtManufactureOrder'] != "" && $_POST['txtQuantityOut'] == "")
{
	$duplicate = false;
	//GET DATA
...
else if( $_REQUEST['btAdd'] && $_POST['txtProductCode'] != "" && $_POST['txtQuantityOut'] != "" && $_POST['txtManufactureOrder'] != "" )
{
	$duplicate = false;
...
//	echo "<script type=\"text/javascript\">";
//	echo "alert(\"Please input ManufactureOrder or QuantityOut \");";
//	echo "window.history.back();";
//	echo "</script>";
// } */
...
<?php
  ...
  $NoRow=1;
    ...
    <td align="center"><input name="EditQuantityOut" type="text" style="text-align:right; width:80px;" id="EditQuantityOut" value="<?=$EditQuantityOut?>" size="10" onkeyup="checkError('EditQuantityOut');" /></td>
