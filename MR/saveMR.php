...
if($txtStockNo == 'NEW')
{
	$sql = "SELECT * FROM `runningnumberpo` WHERE Name = 'MR'";
	...
	$number=$data[1];
	$number++;
	$sql="UPDATE runningnumberpo SET Max = {$number} WHERE Name= 'MR'";
	//echo $sql;
    $result=mysql_query($sql);	 
}
else
...
for ($i=0; $i<=10; $i++)
	{
		if($LineItems[$i]["ProductCode"] != '')
		{
			...
			$sql_line = "INSERT INTO `InventoryLine`(`StockNo`, `ProductCode`,  `Price`,`QuantityOut`,`TotalPrice`, `ManufactureOrder`) VALUES ('{$txtStockNo}','{$ProductCode}','{$UnitPrice}','{$QuantityOut}','{$TotalPrice}','{$MONo}')";
			//echo "<br>";
			//echo "<br>";
			//echo $sql_line;
			$NoRow++;
...
$print ="window.opener.location.href=\"MR.php?StockNo=".$txtStockNo."\";";
