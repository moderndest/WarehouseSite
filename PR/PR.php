<?php
	...
	if($_REQUEST['txtProductCode']!=NULL)	
	{
	 ...
	 if($_REQUEST['PONo']!=NULL)	
	{
		$_SESSION['PONo'] = $_REQUEST['PONo'];
		$InvoiceDate=$_SESSION['txtDate'];
		$Reason = $_SESSION['txtReason'];	
		$WarehouseCode= $_SESSION['txtWarehouse'];
	 	$SupplierCode = $_SESSION['txtSupplierCode'];
...
function disable(id){
  ...
  x.style.backgroundColor='#c0c0c0';
	...
	function getProduct() 
	{
		...
		window.location = "PR.php?P="+ProductCode;
	}
	function getInvoice() 
	{
		var StockNo = document.getElementById("StockNo").value;
		window.location = "PR.php?StockNo="+StockNo;
	}
	function showPOlist() //OPEN RECORD
	{
		window.open("ListPOHead.php","popup","width=400,height=500,scrollbars");
	}
	function getPO() 
	{
		var PONo = document.getElementById("PONo").value;
		window.location = "PR.php?PONo="+PONo;
	}
...
if($OPENING)
	...
	//SELECT NEW LINEITEM
	  $sql = "SELECT P.ProductCode,ProductName,Price,QuantityOut,TotalPrice,PurchaseOrder FROM InventoryLine I JOIN Product P ON I.ProductCode = P.ProductCode WHERE StockNo ='".$StockNo."'";
	  $result=mysql_query($sql);
	  //GET DATA
	...
	{ 
		if(ISSET($_SESSION['StockNo']))
		{
			...
			$_SESSION['Reason'] = "Return Purchase Order";
	}
	if($_SESSION['StockNo']=="NEW")
	{
		$Reason = "Return Purchase Order";
...
<form id="form1" name="form1" method="post" action="">
  ...
  <p>
    <input type="button" value="NEW" onClick="window.location = 'clearPR.php'" />
    <input type="submit" value="SAVE" formaction = "savePR.php" formmethod = "get" formtarget = "_blank"/>
    <input type="submit" name="btnOpen" id="btnOpen" value="OPEN" onClick="window.open('ListofStockPR.php','popup','width=610,height=500,scrollbars');"/>
    <input type="submit" value="DELETE" formaction="deletePR.php" formmethod="get" formtarget="_blank" />
  ...
  <table width="960" border="0">
    ...
    <tr>
      ...
      <td><input type="submit" name="btnCustomer" id="btnCustomer" value="..." onClick="window.open('ListSupplierPR.php','popup','width=610,height=500,scrollbars');" /></td>
      <td>Supplier Name :
    ...
    <tr>
      ...
      <td><input type="submit" name="btnWarehouse" id="btnWarehouse" value="..." onClick="window.open('ListWarehousePR.php','popup','width=610,height=500,scrollbars');" /></td>
      <td>Warehouse Name :
    ...
    if(isset( $_REQUEST['cmd']) && $_REQUEST['cmd']=='Delete') 
{
	...
	//IF CLICK BUTTON ADD
  if(isset($_REQUEST['btAdd']) && $_POST['txtProductCode'] != "" && $_POST['txtQuantity'] != ""&& $_POST['PONo'] != "" )
{
	$duplicate = false;
	//GET DATA
	$newLineItem=array( "ProductCode" => $_REQUEST['txtProductCode'],
						...
						"QuantityOut" => $_REQUEST['txtQuantity'],
						"TotalPrice" => $_REQUEST['txtTotalPrice'],
						"PurchaseOrder" => $_REQUEST['PONo']);	
...
<form name="form2" id="form2" method="post" >
  ...
  <th width="65" scope="col"></th>
      ...
      <th width="147" scope="col"><font color ="red">*</font>Quantity OUT</th>
      <th width="144" scope="col">Total Price</th>
      <th width="185" scope="col"><font color ="red">*</font>PO No.</th>
    </tr>
  ...
  $NoRow=1;
	...
	/*----------------------- EDIT LINEITEM	------------------------------*/
				...
				$EditPONo = $rowData['PurchaseOrder'];
    ...
    <tr>
      ...
      <td align="center"><input name="EditQuantityOut" style="text-align:right; width:120px;" type="text" id="EditQuantityOut"  value="<?=$EditQuantityOut?>"size="10"  onchange="Editcalculateprice();"/>
      </td>
      ...
      <td align="center"><input name="EditPONo" class="bg" style=" width:80px;"type="text" id="EditPONo" value="<?=$EditPONo?>"onchange="getPO(); "/></td>
    </tr>
    <?php
			}
...
?>
    <tr>
      ...
      <td align="center"><?=$rowData['PurchaseOrder']?></td>
    </tr>
    <?php
			}
				$NoRow++;
  ...
  ?>
    ...
    <tr>
      ...
      <input type="submit" name="btnProduct" id="btnProduct" value="..." onClick="window.open('ListProductPR.php','popup','width=610,height=500,scrollbars');"/></td>
      ...
      <input name="txtQuantity"style="text-align:right;" type="text" id="txtQuantity" value="<?=$QuantityOut?>"  onkeyup="checkError('txtQuantity');" onChange="calculateprice();" /></td>
      ...
      <td><label for="PONo"></label>
        <input name="PONo" type="text" id="PONo" value="<?=$PONo?>"onChange="getPO(); "/>
      <input type="submit" name="btnSOCode" id="btnSOCode" value="..."  onclick="showPOlist();"/></td>
    </tr>
  </table>
