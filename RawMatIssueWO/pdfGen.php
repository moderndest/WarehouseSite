...
require('fpdf.php');
	...
	if($choice == 1)
	{
		$sql ="SELECT L.StockNo, L.ProductCode,P.ProductName, L.QuantityOut,WorkOrder
FROM InventoryLine L JOIN Product P ON L.ProductCode = P.ProductCode WHERE L.StockNo = '$StockNo'";
		...
		while($data=mysql_fetch_assoc($result))
				$listData[]=$data;
	//Group By StockNo 			
		$sql="SELECT SUM(L.QuantityOut) FROM InventoryLine L WHERE L.StockNo = '$StockNo'";
...
$header=array('Product Code','Product Name','Quantity Out','WorkOrder');
$pdf->AddPage();
...
$pdf->Cell($w[3],5,$row['WorkOrder'],LR,0,'R',$fill);
...
$pdf->Output();
	...
	else if($choice == 2)
	{
		...
		$sql="SELECT L.StockNo, Date, ProductCode, QuantityOut, WorkOrder
FROM Inventory I
JOIN InventoryLine L ON I.StockNo = L.StockNo WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and (L.StockNo LIKE '%IW%')";
    $result = mysql_query($sql,$conn);

    while($data=mysql_fetch_assoc($result))
				$listData[]=$data;
				
		$sql="SELECT SUM(L.QuantityOut) FROM Inventory I JOIN InventoryLine L ON I.StockNo=L.StockNo WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and (L.StockNo LIKE  '%IW%');" ;
    $sumResult = mysql_query($sql,$conn);
	//list($sumQuantityOut, $sumTotalPrice) = mysql_fetch_row($sumResult);
    list($sumQuantityOut) = mysql_fetch_row($sumResult);
...
$pdf=new FPDF();
//Column titles  InvoiceDate,TotalAmount,VAT,AmountDue
$header=array('Stock No','Date','Product Code','Quantity Out','Work Order');
$pdf->AddPage();
...
//        $pdf->Cell($w[$i],7,$header[$i],'TB',0,'C');
    ...
    foreach($listData as $row)
    {
        ...
        $pdf->Cell($w[4],6,$row['WorkOrder'],LR,0,'R',$fill);
    ...
    $pdf->Cell($w[2],6,'Grand Total',L,0,'C',$fill);
    $pdf->Cell($w[3],6,number_format($sumQuantityOut,2),L,0,'R',$fill);
...
$pdf->Cell(0,7,'No Data Found!!!',0,0,'L');
	...
	{
			$sql ="CREATE or REPLACE VIEW V1 AS SELECT MONTH( S.Date ) , L.ProductCode, SUM( L.QuantityOut ) AS  'TotalQuantityinMonth'
FROM InventoryLine L
JOIN Inventory S ON L.StockNo = S.StockNo
WHERE MONTH( S.Date ) ='$month3'
AND L.StockNo LIKE  'IW%'
