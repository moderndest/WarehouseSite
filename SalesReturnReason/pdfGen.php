...
require('fpdf.php');
	...
	if($choice == 1)
	{
		$sql ="SELECT L.StockNo, L.ProductCode, P.ProductName, L.QuantityIn, L.Price, L.TotalPrice ,L.SaleOrder
FROM inventoryline L JOIN inventory I ON I.StockNo=L.StockNo JOIN product P ON L.ProductCode=P.ProductCode WHERE L.StockNo = '$Stock'";
		...
		$sql="SELECT SUM(L.QuantityIn), SUM(L.TotalPrice) FROM inventoryline L WHERE L.StockNo = '$Stock'";
...
$header=array('Product Code','Name','Quantity IN','Price','Total Price','SaleOrder Number');
$pdf->AddPage();
...
$pdf->Output();
	...
	else if($choice == 2)
	{
		...
		$sql="SELECT I.StockNo,Date,L.ProductCode,L.QuantityIn,L.Price,L.TotalPrice,L.SaleOrder FROM inventory I JOIN inventoryline L ON I.StockNo=L.StockNo WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and (L.StockNo LIKE 'SR%')";
    $result = mysql_query($sql,$conn);

    while($data=mysql_fetch_assoc($result))
				$listData[]=$data;
				
		$sql="SELECT SUM(L.QuantityIn) , SUM(L.TotalPrice ) FROM inventory I JOIN inventoryline L ON I.StockNo=L.StockNo WHERE (month(Date) between '$month1' and '$month2') and (year(Date) between '$year1' and '$year2') and (L.StockNo LIKE  'SR%');";
    $sumResult = mysql_query($sql,$conn);
...
$pdf=new FPDF();
//Column titles  InvoiceDate,TotalAmount,VAT,AmountDue
$header=array('Stock No','Date','Product Code','Quantity In','Unit Price','Total Price','Sale Order');
$pdf->AddPage();
...
//        $pdf->Cell($w[$i],7,$header[$i],'TB',0,'C');
    ...
    foreach($listData as $row)
    {
        ...
        $pdf->Cell($w[3],6,$row['QuantityIn'],L,0,'R',$fill);
    ...
    $pdf->Cell($w[3],6,number_format($sumQuantityIn,2),LR,0,'R',$fill);
...
$pdf->Ln(15);
$pdf->SetFont('Arial','B',15);
$pdf->Cell(0,7,'Month :' .$month3,0,0,'L');
