...
if($txtStockNo == 'NEW')
{
	$sql = "SELECT * FROM runningnumber WHERE Name = 'SL'";
	...
	$Max=$data[1];
	$Max++;
	$sql="UPDATE runningnumber SET Max = {$Max} WHERE Name = 'SL'";
	echo $sql;
    $result=mysql_query($sql);	 
}
else
...
for ($i=0; $i<=10; $i++)
	{
		if($LineItems[$i]["ProductCode"] != '')
		{
			...
			$sql_line = "INSERT INTO `inventoryline`(`StockNo`,`ProductCode`,`Price`, `QuantityOut`, `TotalPrice`, `SaleOrder` ) VALUES ('{$txtStockNo}','{$ProductCode}','{$UnitPrice}','{$QuantityOut}','{$TotalPrice}','{$SOCode}')";
			echo "<br>";
			echo "<br>";
			echo $sql_line;
			$NoRow++;
