...
if($txtStockNo == 'NEW')
{
	$sql = "SELECT * FROM `runningnumberpo` WHERE Name = 'WR'";
	...
	$number=$data[1];
	$number++;
	$sql="UPDATE runningnumberpo SET Max = {$number} WHERE Name= 'WR'";
	//echo $sql;
    $result=mysql_query($sql);	 
}
else
...
for ($i=0; $i<=10; $i++)
	{
		if($LineItems[$i]["ProductCode"] != '')
		{
			...
			$PONo = $LineItems[$i]["WorkOrder"];
			$sql_line = "INSERT INTO `InventoryLine`(`StockNo`, `ProductCode`,  `Price`,`QuantityOut`,`TotalPrice`, `WorkOrder`) VALUES ('{$txtStockNo}','{$ProductCode}','{$UnitPrice}','{$QuantityOut}','{$TotalPrice}','{$PONo}')";
			//echo "<br>";
			//echo "<br>";
			//echo $sql_line;
			$NoRow++;
...
$print ="window.opener.location.href=\"WR.php?StockNo=".$txtStockNo."\";";
